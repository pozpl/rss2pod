...
sub add_item_to_feed() {
	...
	my $feed_id_items_key =
	  $self->get_filled_key( 'FEED_ID_ITEMS', { "id" => $feed_id } );
	$self->redis_connection->lpush( $feed_id_items_key, $item_seriaiysed );

	my $feed_id_items_hash_zset_key =
	  $self->get_filled_key( 'FEED_ID_ITEMS_HASH_ZSET', { "id" => $feed_id } );
	my $item_hash = md5_hex($item_seriaiysed);

	my $feed_id_items_count_key =
	  $self->get_filled_key( 'FEED_ID_ITEMS_COUNT', { "id" => $feed_id } );
	my $item_count = $self->redis_connection->incr($feed_id_items_count_key);
	$self->redis_connection->zadd( $feed_id_items_hash_zset_key, $item_count,
		$item_hash );
}
...
sub is_item_alrady_in_feed() {
	...
	my $item_hash = md5_hex($item_seriaiysed);
	my $item_score =
	  $self->redis_connection->zscore( $feed_id_items_hash_zset_key, $item_hash );
...
sub create_feed_for_url() {
	...
	my $feed_url_id_key =
	  $self->get_filled_key( 'FEED_URL_ID', { 'url' => md5_hex($url) } );
	...
	if ( !defined $ret_feed_id ) {
		...
		my $feed_id_title_key =
		  $self->get_filled_key( 'FEED_ID_TITLE', { "id" => $new_feed_id } );
		$self->redis_connection->set( $feed_id_title_key, $url );

		my $feed_id_url_key =
		  $self->get_filled_key( 'FEED_ID_URL', { "id" => $new_feed_id } );
		$self->redis_connection->set( $feed_id_url_key, $url );

		$self->redis_connection->set( $feed_url_id_key, $new_feed_id );

		my $feed_id_items_shift =
		  $self->get_filled_key( 'FEED_ID_ITEMS_SHIFT', { "id" => $new_feed_id } );
...
sub del_and_get_old_items_from_feed() {
	...
	if ( $max_items_in_feed < $list_length ) {
		...
		@dead_items =
		  $self->redis_connection->lrange( $feed_id_items_key, $max_items_in_feed, -1 );
		my $redis_ok =
		  $self->redis_connection->ltrim( $feed_id_items_key, 0, $max_items_in_feed - 1 );
		my $feed_id_items_shift_key =
		  $self->get_filled_key( 'FEED_ID_ITEMS_SHIFT', { "id" => $feed_id } );
	...
	if ( $md5_zset_length > $max_items_in_feed ) {
		...
		$self->redis_connection->zremrangebyrank( $feed_id_items_hash_zset_key, 0,
			( $md5_zset_length - $max_items_in_feed ) );
	}
...
sub del_feed() {
	...
	if ( $feed_identificator =~ /{d+}/xms ) {
		...
		$feed_url_id_key =
		  $self->get_filled_key( 'FEED_URL_ID', { 'url' => md5_hex($feed_url) } );
	...
	my $feed_id_wild_key =
	  $self->get_filled_key_wildcard( 'FEED_ID', { 'id' => $feed_id } );
...
#22
sub del_and_get_old_podcasts_from_podlist() {
}    #trimm old items from user podlist, and get all trimmed items
...
sub add_new_user() {
	...
	my $user_id_login_key =
	  $self->get_filled_key( 'USER_ID_LOGIN', { "id" => $new_user_id } );
	$set_ok = $self->redis_connection->set( $user_id_login_key, $login );

	my $user_id_email_key =
	  $self->get_filled_key( 'USER_ID_EMAIL', { "id" => $new_user_id } );
	$set_ok = $self->redis_connection->set( $user_id_email_key, $email );

	my $user_id_pass_key =
	  $self->get_filled_key( 'USER_ID_PASS', { "id" => $new_user_id } );
...
sub delete_user() {
	...
	if ( $user_identificator =~ /{d+}/xms ) {
		...
		my $user_id_login_key =
		  $self->get_filled_key( 'USER_ID_LOGIN', { "id" => $user_id } );
		$user_login = $self->redis_connection->get($user_id_login_key);
		$id_login_key =
		  $self->get_filled_key( 'ID_LOGIN', { "login" => md5_hex($user_login) } );
	}
	else {
	...
	my $user_id_wild_key =
	  $self->get_filled_key_wildcard( 'USER_ID', { 'id' => $user_id } );
...
sub is_user_exists() {
	...
	my $user_exists =
	  ( defined $user_id && $user_id != 0 && !( $user_login eq "" ) ) ? 1 : 0;
...
#29
############################################
# Usage      : uspdate_user_password($user_login, $new_password);
# Purpose    : update user's password
# Returns    : none
# Parameters : user login- string, new_password string
# Throws     : no exceptions
# Comments   : ???
# See Also   : n/a
sub update_user_password() {
	my ( $self, $user_login, $new_password ) = @_;
	...
	if ( defined $user_id && $user_id >= 0 ) {
		my $user_id_pass_key =
		  $self->get_filled_key( 'USER_ID_PASS', { "id" => $user_id } );
		$set_ok =
		  $self->redis_connection->set( $user_id_pass_key, md5_hex($new_password) );
	}
...
#30
############################################
# Usage      : is_user_password_valid($user_login, $password);
# Purpose    : check if given password id suited for stored one
# Returns    : true if password is valid, false otherwise
# Parameters : user login- string, password string
# Throws     : no exceptions
# Comments   : ???
# See Also   : n/a
sub is_user_password_valid() {
	my ( $self, $user_login, $password ) = @_;
	my $id_login_key =
	  $self->get_filled_key( 'ID_LOGIN', { "login" => md5_hex($user_login) } );
	my $user_id = $self->redis_connection->get($id_login_key);

	my $password_from_storage = "";
	if ( defined $user_id && $user_id >= 0 ) {
		my $user_id_pass_key =
		  $self->get_filled_key( 'USER_ID_PASS', { "id" => $user_id } );
		$password_from_storage = $self->redis_connection->get($user_id_pass_key);
	}
	my $is_valid =
	   !( md5_hex($password) eq $password_from_storage ) ? 0
	  : ( $password          eq "" )                     ? 0
	  :                                                    1;
...
#31
############################################
# Usage      : uspdate_user_email($user_login, $new_email);
# Purpose    : update user's email
# Returns    : none
# Parameters : user login- string, new_email string
# Throws     : no exceptions
# Comments   : ???
# See Also   : n/a
sub update_user_email() {
	my ( $self, $user_login, $new_email ) = @_;
	...
	if ( defined $user_id && $user_id >= 0 ) {
		my $user_id_email_key =
		  $self->get_filled_key( 'USER_ID_EMAIL', { "id" => $user_id } );
		$set_ok = $self->redis_connection->set( $user_id_email_key, $new_email );
	}
...
#32
############################################
# Usage      : $email = get_user_email($user_login);
# Purpose    : get email for user name
...
sub get_user_email() {
	...
	my $email_from_storage = "";
	if ( defined $user_id && $user_id >= 0 ) {
		my $user_id_email_key =
		  $self->get_filled_key( 'USER_ID_EMAIL', { "id" => $user_id } );
		$email_from_storage = $self->redis_connection->get($user_id_email_key);
	}

	return $email_from_storage;
}

#33
###########################################
# Usage      : add_user_podcast($user_login, $podcast_name);
# Purpose    : add new podcast to user account
# Returns    : none
# Parameters : user login- string, podcast_name - string
# Throws     : no exceptions
# Comments   : ???
# See Also   : n/a
sub add_user_podcast() {
